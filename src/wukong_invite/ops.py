...
def cmd_extract_code(image: str) -> int:
    ...
    text = VisionOCR(project_root).recognize_text(Path(image))
...
def cmd_fill_app(code: str, no_submit: bool) -> int:
    submit_line = (
        '        click button "立即体验" of UI element 1 of scroll area 1 of group 1 of group 1 of front window'
        if not no_submit
        else ""
    )
    script = f"""
...
tell application "System Events"
    tell process "DingTalkReal"
        ...
        click text field 1 of UI element 1 of scroll area 1 of group 1 of group 1 of front window
        delay 0.2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    fill_parser = subparsers.add_parser("fill-app", help="Fill the invite code into Wukong.app")
